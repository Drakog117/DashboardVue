...
export function renderGraficaCrecimiento() {
    var myChart = echarts.init(document.getElementById('crecimiento'));

    fetch('http://localhost/phpDash/getData.php')
...
function fetchData(callback) {
    ...
    xhr.open("GET", "http://localhost/phpDash/getData.php", true);
    xhr.send();
}
...
export function actualizarVelocimetro(url) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
...
actualizarVelocimetro('http://localhost/phpDash/obtener_metas.php');
